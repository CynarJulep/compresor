...
Streamlit Community Cloud **hiberna** la app tras **~12 horas sin tráfico de usuario**. Dentro de un iframe de Google Sites **no se puede despertar** (el botón de wake no funciona bien embebido). El workflow `.github/workflows/keep-alive.yml` abre la app con un navegador real (Playwright) cada ~10 horas — un `curl` no alcanza porque solo sirve HTML estático. También se puede disparar a mano en **Actions → Keep Streamlit awake → Run workflow**.
